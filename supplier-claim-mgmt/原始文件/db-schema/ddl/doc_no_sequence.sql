...
--
-- 生成说明：
--   1. 权威来源：docs/detail-design/详细设计-单据编码.md「流水维度与并发唯一方案」、docs/conventions.md S7、
--      docs/db-schema/表结构文档.md「单据编码」段；本文件严格按 LLD 序列表字段定义翻译，不增不减列。
...
--      将同「前缀+日期」的并发取号串行化，不同前缀/不同日期落不同行、不互锁（见 LLD 取号流程）。

-- ===== doc_no_sequence 单据流水序列表 =====
CREATE TABLE `doc_no_sequence` (
  `id` VARCHAR(32) NOT NULL COMMENT '主键，雪花算法',
  `prefix` VARCHAR(8) NOT NULL COMMENT '单据前缀：SPCO/SXCO/SPJS/SXJS/JSFK/DKD',
